...
void vlog_category_profile(vlog_category_t *a_category, int flag)
{
	...
	vg_assert(a_category,);
	vg_profile(flag, "--category[%p][%s][%p]--",
			a_category,
			a_category->name,
			a_category->fit_rules);
	if (a_category->fit_rules) {
		vg_arraylist_foreach(a_category->fit_rules, i, a_rule) {
...
{
	vg_assert(a_category,);
	if (a_category->fit_rules) vg_arraylist_del(a_category->fit_rules);
	vg_debug("vlog_category_del[%p]", a_category);
    free(a_category);
	return;
}
...
static int vlog_category_obtain_rules(vlog_category_t * a_category, vg_arraylist_t * rules)
{
	int i;
	int count = 0;
	int fit = 0;
	...
	a_category->fit_rules = vg_arraylist_new(NULL);
	if (!(a_category->fit_rules)) {
	...
	vg_arraylist_foreach(rules, i, a_rule) {
		fit = vlog_rule_match_category(a_rule, a_category->name);
		if (fit) {
			if (vg_arraylist_add(a_category->fit_rules, a_rule)) {
				vg_error("vg_arrylist_add fail");
				goto err;
			}
	...
	if (count == 0) {
		if (wastebin_rule) {
			vg_debug("category[%s], no match rules, use wastebin_rule", a_category->name);
			if (vg_arraylist_add(a_category->fit_rules, wastebin_rule)) {
				vg_error("vg_arrylist_add fail");
				goto err;
			}
			...
			vg_debug("category[%s], no match rules & no wastebin_rule", a_category->name);
		}
	}

	return 0;
err:
	vg_arraylist_del(a_category->fit_rules);
...
vlog_category_t *vlog_category_new(const char *name, vg_arraylist_t * rules)
{
	...
	vg_assert(name, NULL);
	vg_assert(rules, NULL);

	len = strlen(name);
	if (len > sizeof(a_category->name) - 1) {
		vg_error("name[%s] too long", name);
		return NULL;
	}
	...
	if (vlog_category_obtain_rules(a_category, rules)) {
		vg_error("vlog_category_fit_rules fail");
		goto err;
	}

	vlog_category_profile(a_category, vg_DEBUG);
	return a_category;
err:
...
{
	vg_assert(a_category, -1);
	vg_assert(new_rules, -1);

	/* 1st, mv fit_rules fit_rules_backup */
	if (a_category->fit_rules_backup) vg_arraylist_del(a_category->fit_rules_backup);
	...
	if (vlog_category_obtain_rules(a_category, new_rules)) {
		vg_error("vlog_category_obtain_rules fail");
		a_category->fit_rules = NULL;
		return -1;
	}
...
void vlog_category_commit_rules(vlog_category_t * a_category)
{
	...
	if (!a_category->fit_rules_backup) {
		vg_warn("a_category->fit_rules_backup is NULL, never update before");
		return;
	}

	vg_arraylist_del(a_category->fit_rules_backup);
...
void vlog_category_rollback_rules(vlog_category_t * a_category)
{
	vg_assert(a_category,);
	if (!a_category->fit_rules_backup) {
		vg_warn("a_category->fit_rules_backup in NULL, never update before");
		return;
	}
