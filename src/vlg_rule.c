...
void vlog_rule_profile(vlog_rule_t * a_rule, int flag)
{
	...
	vlg_assert(a_rule,);
	vlg_profile(flag, "---rule:[%p][%s%c%d]-[%d,%d][%s,%p,%d:%ld*%d~%s][%d][%d][%s:%s:%p];[%p]---",
		a_rule,
	...
	if (a_rule->dynamic_specs) {
		vlg_arraylist_foreach(a_rule->dynamic_specs, i, a_spec) {
...
static int vlog_rule_output_static_file_single(vlog_rule_t * a_rule, vlog_thread_t * a_thread)
{
	...
	if (vlog_format_gen_msg(a_rule->format, a_thread)) {
		vlg_error("vlog_format_gen_msg fail");
		return -1;
	}
	...
	if (stat(a_rule->file_path, &stb)) {
		if (errno != ENOENT) {
			vlg_error("stat fail on [%s], errno[%d]", a_rule->file_path, errno);
			return -1;
		} else {
			do_file_reload = 1;
	...
	if (do_file_reload) {
		...
		if (a_rule->static_fd < 0) {
			vlg_error("open file[%s] fail, errno[%d]", a_rule->file_path, errno);
			return -1;
		}
		...
		if (redo_inode_stat) {
			if (stat(a_rule->file_path, &stb)) {
				vlg_error("stat fail on new file[%s], errno[%d]", a_rule->file_path, errno);
				return -1;
			}
		}
	...
	if (write(a_rule->static_fd,
		...
		vlg_error("write fail, errno[%d]", errno);
		return -1;
	}
	...
	if (a_rule->fsync_period && ++a_rule->fsync_count >= a_rule->fsync_period) {
		...
		if (fsync(a_rule->static_fd)) {
			vlg_error("fsync[%d] fail, errno[%d]", a_rule->static_fd, errno);
		}
	}
...
static char * vlog_rule_gen_archive_path(vlog_rule_t *a_rule, vlog_thread_t *a_thread)
{
	...
	vlg_arraylist_foreach(a_rule->archive_specs, i, a_spec) {
		if (vlog_spec_gen_archive_path(a_spec, a_thread)) {
			vlg_error("vlog_spec_gen_path fail");
			return NULL;
		}
	}
...
static int vlog_rule_output_static_file_rotate(vlog_rule_t * a_rule, vlog_thread_t * a_thread)
{
	...
	if (a_rule->fsync_period && ++a_rule->fsync_count >= a_rule->fsync_period) {
		a_rule->fsync_count = 0;
		if (fsync(fd)) vlg_error("fsync[%d] fail, errno[%d]", fd, errno);
	}

	if (close(fd) < 0) {
		vlg_error("close fail, maybe cause by write, errno[%d]", errno);
		return -1;
	}

	if (len > a_rule->archive_max_size) {
		vlg_debug("one msg's len[%ld] > archive_max_size[%ld], no rotate",
			 (long)len, (long)a_rule->archive_max_size);
		return 0;
	}

	if (stat(a_rule->file_path, &info)) {
		vlg_warn("stat [%s] fail, errno[%d], maybe in rotating", a_rule->file_path, errno);
		return 0;
	}
	...
	if (vlog_rotater_rotate(vlog_env_conf->rotater, 
		...
		vlg_error("vlog_rotater_rotate fail");
		return -1;
...
static int vlog_rule_output_dynamic_file_single(vlog_rule_t * a_rule, vlog_thread_t * a_thread)
{
	...
	if (vlog_format_gen_msg(a_rule->format, a_thread)) {
		vlg_error("vlog_format_output fail");
		return -1;
	}
	...
	fd = open(vlog_buf_str(a_thread->path_buf),
		a_rule->file_open_flags | O_WRONLY | O_APPEND | O_CREAT, a_rule->file_perms);
	if (fd < 0) {
		vlg_error("open file[%s] fail, errno[%d]", vlog_buf_str(a_thread->path_buf), errno);
		return -1;
	}
...
static int vlog_rule_output_dynamic_file_rotate(vlog_rule_t * a_rule, vlog_thread_t * a_thread)
{
	...
	if (close(fd) < 0) {
		vlg_error("write fail, maybe cause by write, errno[%d]", errno);
		return -1;
	}
	...
	if (stat(path, &info)) {
		vlg_warn("stat [%s] fail, errno[%d], maybe in rotating", path, errno);
		return 0;
	}
...
static int vlog_rule_output_static_record(vlog_rule_t * a_rule, vlog_thread_t * a_thread)
{
	...
	if (!a_rule->record_func) {
		vlg_error("user defined record funcion for [%s] not set, no output",
			a_rule->record_name);
		return -1;
	}
...
static int syslog_facility_atoi(char *facility)
{
	...
	vlg_error("wrong syslog facility[%s], must in LOG_LOCAL[0-7] or LOG_USER", facility);
	return -187;
}

static int vlog_rule_parse_path(char *path_start, /* start with a " */
		char *path_str, size_t path_size, vlg_arraylist_t **path_specs,
		int *time_cache_count)
{
	char *p, *q;
	size_t len;
	vlog_spec_t *a_spec;
	vlg_arraylist_t *specs;

	p = path_start + 1;

	q = strrchr(p, '"');
	if (!q) {
		vlg_error("matching \" not found in conf line[%s]", path_start);
		return -1;
	}
	len = q - p;
	if (len > path_size - 1) {
		vlg_error("file_path too long %ld > %ld", len, path_size - 1);
		return -1;
	}
	memcpy(path_str, p, len);

	/* replace any environment variables like %E(HOME) */
	if (vlg_str_replace_env(path_str, path_size)) {
	...
	specs = vlg_arraylist_new((vlg_arraylist_del_fn)vlog_spec_del);
	if (!path_specs) {
	...
	for (p = path_str; *p != '\0'; p = q) {
		...
		if (vlg_arraylist_add(specs, a_spec)) {
	...
	*path_specs = specs;
	return 0;
err:
	if (specs) vlg_arraylist_del(specs);
		...
		int * time_cache_count)
{
	...
	vlg_assert(default_format, NULL);
	vlg_assert(formats, NULL);
	...
	if (nscan != 1) {
		vlg_error("sscanf [%s] fail, selector", line);
		goto err;
	}
	action = line + nread;
	...
	if (nscan != 2) {
		vlg_error("sscanf [%s] fail, category or level is null",
			 selector);
		goto err;
	}
	...
	for (p = category; *p != '\0'; p++) {
		if ((!isalnum(*p)) && (*p != '_') && (*p != '-') && (*p != '*') && (*p != '!')) {
			vlg_error("category name[%s] character is not in [a-Z][0-9][_!*-]", category);
			goto err;
		}
	}
	...
	if (nscan < 1) {
		vlg_error("sscanf [%s] fail", action);
		goto err;
	}

	/* check and get format */
	if (STRCMP(format_name, ==, "")) {
		vlg_debug("no format specified, use default");
		...
		vlg_arraylist_foreach(formats, i, a_format) {
		...
		if (!find_flag) {
			vlg_error("in conf file can't find format[%s], pls check",
			     format_name);
			goto err;
		}
	}
	...
	case '-' :
		...
		if (file_path[1] != '"') {
			vlg_error(" - must set before a file output");
			goto err;
		}
	...
	case '"' :
		...
		if (rc) {
			vlg_error("vlog_rule_parse_path fail");
			goto err;
		}
		...
		if (file_limit) {
			...
			if (nscan) {
				a_rule->archive_max_size = vlg_parse_byte_size(archive_max_size);
			}
			p = strchr(file_limit, '"');
			if (p) { /* archive file path exist */
				...
				if ( (p == NULL) || ((strchr(p, 'r') == NULL) && (strchr(p, 's') == NULL))) {
					vlg_error("archive_path must contain #r or #s");
					goto err;
				}
			}
		}
		...
		if (a_rule->dynamic_specs) {
			...
			if (fstat(a_rule->static_fd, &stb)) {
				vlg_error("stat [%s] fail, errno[%d], failing to open static_fd", a_rule->file_path, errno);
				goto err;
			}
	...
	case '|' :
		...
		if (!a_rule->pipe_fp) {
			vlg_error("popen fail, errno[%d]", errno);
			goto err;
		}
		a_rule->pipe_fd = fileno(a_rule->pipe_fp);
		if (a_rule->pipe_fd < 0 ) {
			vlg_error("fileno fail, errno[%d]", errno);
			goto err;
		}
	...
	case '>' :
		if (STRNCMP(file_path + 1, ==, "syslog", 6)) {
			...
			vlg_error("[%s]the string after is not syslog, stdout or stderr", output);
			goto err;
		}
		break;
	case '$' :
		...
		if (file_limit) {  /* record path exists */
			p = strchr(file_limit, '"');
			if (!p) {
				vlg_error("record_path not start with \", [%s]", file_limit);
				goto err;
			}
			p++; /* skip 1st " */
			...
			q = strrchr(p, '"');
			if (!q) {
				vlg_error("matching \" not found in conf line[%s]", p);
				goto err;
			}
			len = q - p;
			if (len > sizeof(a_rule->record_path) - 1) {
				vlg_error("record_path too long %ld > %ld", len, sizeof(a_rule->record_path) - 1);
				goto err;
			}
		...
		/* replace any environment variables like %E(HOME) */
		rc = vlg_str_replace_env(a_rule->record_path, sizeof(a_rule->record_path));
		if (rc) {
			vlg_error("vlg_str_replace_env fail");
			goto err;
		}
		...
		if (strchr(a_rule->record_path, '%') == NULL) {
			...
			a_rule->dynamic_specs = vlg_arraylist_new((vlg_arraylist_del_fn)vlog_spec_del);
			if (!(a_rule->dynamic_specs)) {
				vlg_error("vlg_arraylist_new fail");
				goto err;
			}
			for (p = a_rule->record_path; *p != '\0'; p = q) {
				...
				rc = vlg_arraylist_add(a_rule->dynamic_specs, a_spec);
				if (rc) {
		...
		break;
	default :
		vlg_error("the 1st char[%c] of file_path[%s] is wrong",
...
void vlog_rule_del(vlog_rule_t * a_rule)
{
	vlg_assert(a_rule,);
	if (a_rule->dynamic_specs) {
		vlg_arraylist_del(a_rule->dynamic_specs);
	...
	if (a_rule->pipe_fp) {
		if (pclose(a_rule->pipe_fp) == -1) {
			vlg_error("pclose fail, errno[%d]", errno);
		}
	}
	if (a_rule->archive_specs) {
		vlg_arraylist_del(a_rule->archive_specs);
		a_rule->archive_specs = NULL;
	}
	vlg_debug("vlog_rule_del[%p]", a_rule);
    free(a_rule);
	return;
}
...
int vlog_rule_set_record(vlog_rule_t * a_rule, vlg_hashtable_t *records)
{
	...
	a_record = vlg_hashtable_get(records, a_rule->record_name);
	if (a_record) {
