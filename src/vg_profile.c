...
#include "vg_profile.h"
#include "vg_xplatform.h"

static void vg_time(char *time_str, size_t time_str_size)
{
	time_t tt;
...
int vg_profile_inner(int flag, const char *file, const long line, const char *fmt, ...)
{
	va_list args;
	char time_str[20 + 1];
	FILE *fp = NULL;
	...
	case vg_DEBUG:
		...
		if (!fp) return -1;
		vg_time(time_str, sizeof(time_str));
