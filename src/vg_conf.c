...
void vlog_conf_profile(vlog_conf_t * a_conf, int flag)
{
	...
	vg_assert(a_conf,);
	vg_profile(flag, "-conf[%p]-", a_conf);
	vg_profile(flag, "--global--");
	vg_profile(flag, "---file[%s],mtime[%s]---", a_conf->file, a_conf->mtime);
	vg_profile(flag, "---in-memory conf[%s]---", a_conf->cfg_ptr);
	vg_profile(flag, "---strict init[%d]---", a_conf->strict_init);
	vg_profile(flag, "---buffer min[%ld]---", a_conf->buf_size_min);
	vg_profile(flag, "---buffer max[%ld]---", a_conf->buf_size_max);
	if (a_conf->default_format) {
		vg_profile(flag, "---default_format---");
		vlog_format_profile(a_conf->default_format, flag);
	}
	vg_profile(flag, "---file perms[0%o]---", a_conf->file_perms);
	vg_profile(flag, "---reload conf period[%ld]---", a_conf->reload_conf_period);
	vg_profile(flag, "---fsync period[%ld]---", a_conf->fsync_period);

	vg_profile(flag, "---rotate lock file[%s]---", a_conf->rotate_lock_file);
	if (a_conf->rotater) vlog_rotater_profile(a_conf->rotater, flag);

	if (a_conf->levels) vlog_level_list_profile(a_conf->levels, flag);

	if (a_conf->formats) {
		vg_profile(flag, "--format list[%p]--", a_conf->formats);
		vg_arraylist_foreach(a_conf->formats, i, a_format) {
			vlog_format_profile(a_format, flag);
		}
	}

	if (a_conf->rules) {
		vg_profile(flag, "--rule_list[%p]--", a_conf->rules);
		vg_arraylist_foreach(a_conf->rules, i, a_rule) {
...
void vlog_conf_del(vlog_conf_t * a_conf)
{
	...
	if (a_conf->formats) vg_arraylist_del(a_conf->formats);
	if (a_conf->rules) vg_arraylist_del(a_conf->rules);
	free(a_conf);
	vg_debug("vlog_conf_del[%p]");
	return;
}
...
vlog_conf_t *vlog_conf_new(const char *config)
{
	...
	if (config && config[0] != '\0' && config[0] != '[') {
		...
		if (nwrite < 0 || nwrite >= sizeof(a_conf->file)) {
			vg_error("not enough space for configurations, nwrite=[%d], errno[%d]", nwrite, errno);
			goto err;
		}
	} else {
		memset(a_conf->file, 0x00, sizeof(a_conf->file));
	...
	if (nwrite < 0 || (nwrite >= sizeof(a_conf->file) && cfg_source == FILE_CFG)) {
		vg_error("not enough space for path name, nwrite=[%d], errno[%d]", nwrite, errno);
		goto err;
	}
	...
	if (!a_conf->levels) {
		vg_error("vlog_level_list_new fail");
		goto err;
	}

	a_conf->formats = vg_arraylist_new((vg_arraylist_del_fn) vlog_format_del);
	if (!a_conf->formats) {
		vg_error("vg_arraylist_new fail");
		goto err;
	}

	a_conf->rules = vg_arraylist_new((vg_arraylist_del_fn) vlog_rule_del);
	if (!a_conf->rules) {
		vg_error("init rule_list fail");
		goto err;
	}

	if (cfg_source == FILE_CFG) {
		if (vlog_conf_build_with_file(a_conf)) {
			vg_error("vlog_conf_build_with_file fail");
			goto err;
		}
	} else if (cfg_source == IN_MEMORY_CFG) {
		if(vlog_conf_build_with_in_memory(a_conf)){
			vg_error("vlog_conf_build_with_in_memory fail");
			goto err;
		}
	} else {
		if (vlog_conf_build_without_file(a_conf)) {
			vg_error("vlog_conf_build_without_file fail");
			goto err;
		}
	}

	vlog_conf_profile(a_conf, vg_DEBUG);
	return a_conf;
err:
...
static int vlog_conf_build_without_file(vlog_conf_t * a_conf)
{
	...
	if (!a_conf->default_format) {
		vg_error("vlog_format_new fail");
		return -1;
	}

	a_conf->rotater = vlog_rotater_new(a_conf->rotate_lock_file);
	if (!a_conf->rotater) {
		vg_error("vlog_rotater_new fail");
		return -1;
	}
	...
	if (!default_rule) {
		vg_error("vlog_rule_new fail");
		return -1;
	}

	/* add default rule */
	if (vg_arraylist_add(a_conf->rules, default_rule)) {
...
static int vlog_conf_build_with_file(vlog_conf_t * a_conf)
{
	...
	if (lstat(a_conf->file, &a_stat)) {
		vg_error("lstat conf file[%s] fail, errno[%d]", a_conf->file,
			 errno);
		return -1;
	}
	...
	if ((fp = fopen(a_conf->file, "r")) == NULL) {
		vg_error("open configure file[%s] fail", a_conf->file);
		return -1;
	}
	...
	while (fgets((char *)pline, sizeof(line) - (pline - line), fp) != NULL) {
		...
		*++p = '\0';
		...
		if (rc < 0) {
			vg_error("parse configure file[%s]line_no[%ld] fail", a_conf->file, line_no);
			vg_error("line[%s]", line);
			goto exit;
		} else if (rc > 0) {
			vg_warn("parse configure file[%s]line_no[%ld] fail", a_conf->file, line_no);
			vg_warn("line[%s]", line);
			vg_warn("as strict init is set to false, ignore and go on");
			rc = 0;
			continue;
		}
	}
...
static int vlog_conf_build_with_in_memory(vlog_conf_t * a_conf)
{
	...
	while (pline != NULL) {
		...
		if (rc < 0) {
			vg_error("parse in-memory configurations[%s] line [%s] fail", a_conf->cfg_ptr, pline);
			break;
		} else if (rc > 0) {
			vg_error("parse in-memory configurations[%s] line [%s] fail", a_conf->cfg_ptr, pline);
			vg_warn("as strict init is set to false, ignore and go on");
...
static int vlog_conf_parse_line(vlog_conf_t * a_conf, char *line, int *section)
{
	...
	if (strlen(line) > MAXLEN_CFG_LINE) {
		vg_error ("line_len[%ld] > MAXLEN_CFG_LINE[%ld], may cause overflow",
	...
	if (line[0] == '[') {
		...
		if (STRCMP(name, ==, "global")) {
			...
			*section = 4;
			...
			vg_error("wrong section name[%s]", name);
			return -1;
		}
		/* check the sequence of section, must increase */
		if (last_section >= *section) {
			vg_error("wrong sequence of section, must follow global->levels->formats->rules");
			return -1;
		}
		...
		if (*section == 4) {
			if (a_conf->reload_conf_period != 0
				 ...
				 * so make it zero.
				 */
				vg_warn("fsync_period[%ld] >= reload_conf_period[%ld],"
	...
	case 1:
		...
		if (nscan != 2) {
			vg_error("sscanf [%s] fail, name or value is null", line);
			return -1;
		}
		...
		if (STRCMP(word_1, ==, "strict") && STRCMP(word_2, ==, "init")) {
			...
			a_conf->buf_size_min = vg_parse_byte_size(value);
		} else if (STRCMP(word_1, ==, "buffer") && STRCMP(word_2, ==, "max")) {
			a_conf->buf_size_max = vg_parse_byte_size(value);
			...
			a_conf->reload_conf_period = vg_parse_byte_size(value);
		} else if (STRCMP(word_1, ==, "fsync") && STRCMP(word_2, ==, "period")) {
			a_conf->fsync_period = vg_parse_byte_size(value);
		} else {
			vg_error("name[%s] is not any one of global options", name);
			if (a_conf->strict_init) return -1;
		}
		break;
	case 2:
		if (vlog_level_list_set(a_conf->levels, line)) {
			vg_error("vlog_level_list_set fail");
			if (a_conf->strict_init) return -1;
		}
		break;
	case 3:
		...
		if (!a_format) {
			vg_error("vlog_format_new fail [%s]", line);
			if (a_conf->strict_init) return -1;
			else break;
		}
		if (vg_arraylist_add(a_conf->formats, a_format)) {
	...
	case 4:
		...
		if (!a_rule) {
			vg_error("vlog_rule_new fail [%s]", line);
			if (a_conf->strict_init) return -1;
			else break;
		}
		if (vg_arraylist_add(a_conf->rules, a_rule)) {
