...
{
	//vg_assert(a_buf,);
	vg_profile(flag, "---buf[%p][%ld-%ld][%ld][%s][%p:%ld]---",
			a_buf,
...
vlog_buf_t *vlog_buf_new(size_t buf_size_min, size_t buf_size_max, const char *truncate_str)
{
	...
	if (buf_size_min == 0) {
		vg_error("buf_size_min == 0, not allowed");
		return NULL;
	}

	if (buf_size_max != 0 && buf_size_max < buf_size_min) {
		vg_error("buf_size_max[%lu] < buf_size_min[%lu] && buf_size_max != 0",
	...
	if (truncate_str) {
		if (strlen(truncate_str) > sizeof(a_buf->truncate_str) - 1) {
			vg_error("truncate_str[%s] overflow", truncate_str);
			goto err;
		} else {
	...
	//vlog_buf_profile(a_buf, vg_DEBUG);
	return a_buf;
...
static int vlog_buf_resize(vlog_buf_t * a_buf, size_t increment)
{
	...
	if (a_buf->size_max != 0 && a_buf->size_real >= a_buf->size_max) {
		vg_error("a_buf->size_real[%ld] >= a_buf->size_max[%ld]",
...
int vlog_buf_vprintf(vlog_buf_t * a_buf, const char *format, va_list args)
{
	...
	if (!a_buf->start) {
		vg_error("pre-use of vlog_buf_resize fail, so can't convert");
		return -1;
	}
	...
	if (nwrite >= 0 && nwrite < size_left) {
		...
		vg_error("vsnprintf fail, errno[%d]", errno);
		vg_error("nwrite[%d], size_left[%ld], format[%s]", nwrite, size_left, format);
		return -1;
	} else if (nwrite >= size_left) {
		int rc;
		//vg_debug("nwrite[%d]>=size_left[%ld],format[%s],resize", nwrite, size_left, format);
		rc = vlog_buf_resize(a_buf, nwrite - size_left + 1);
		if (rc > 0) {
			vg_error("conf limit to %ld, can't extend, so truncate", a_buf->size_max);
			va_copy(ap, args);
			size_left = a_buf->end_plus_1 - a_buf->tail;
			...
			vg_error("vlog_buf_resize fail");
			return -1;
		} else {
			//vg_debug("vlog_buf_resize succ, to[%ld]", a_buf->size_real);
...
int vlog_buf_printf_dec32(vlog_buf_t * a_buf, uint32_t ui32, int width)
{
	...
	if ((q = a_buf->tail + out_len) > a_buf->end) {
		int rc;
		//vg_debug("size_left not enough, resize");
		rc = vlog_buf_resize(a_buf, out_len - (a_buf->end - a_buf->tail));
		if (rc > 0) {
			size_t len_left;
			vg_error("conf limit to %ld, can't extend, so output", a_buf->size_max);
...
int vlog_buf_append(vlog_buf_t * a_buf, const char *str, size_t str_len)
{
	...
	if ((p = a_buf->tail + str_len) > a_buf->end) {
		...
		if (rc > 0) {
			size_t len_left;
			vg_error("conf limit to %ld, can't extend, so output",
				 a_buf->size_max);
			len_left = a_buf->end - a_buf->tail;
