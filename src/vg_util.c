...
size_t vg_parse_byte_size(char *astring)
{
	...
	*q = '\0';
	...
	default:
		if (!isdigit(c)) {
			vg_error("Wrong suffix parsing " "size in bytes for string [%s], ignoring suffix",
				 astring);
		}
		break;
	}
...
int vg_str_replace_env(char *str, size_t str_size)
{
	...
	do {
		...
		if (str_len > str_size - 1) {
			vg_error("repalce env_value[%s] cause overlap", env_value);
			return -1;
		}
