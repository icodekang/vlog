...
size_t vlg_parse_byte_size(char *astring)
{
	...
	*q = '\0';
	...
	default:
		if (!isdigit(c)) {
			vlg_error("Wrong suffix parsing " "size in bytes for string [%s], ignoring suffix",
				 astring);
		}
		break;
	}
...
int vlg_str_replace_env(char *str, size_t str_size)
{
	...
	do {
		...
		if (str_len > str_size - 1) {
			vlg_error("repalce env_value[%s] cause overlap", env_value);
			return -1;
		}
