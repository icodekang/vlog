...
{
	vlg_assert(a_event,);
	vlg_profile(flag, "---event[%p][%s,%s][%s(%ld),%s(%ld),%ld,%d][%p,%s][%ld,%ld][%ld,%ld][%d]---",
			a_event,
...
void vlog_event_del(vlog_event_t * a_event)
{
	...
	vlg_debug("vlog_event_del[%p]", a_event);
    free(a_event);
	return;
}
...
vlog_event_t *vlog_event_new(int time_cache_count)
{
	...
	if (gethostname(a_event->host_name, sizeof(a_event->host_name) - 1)) {
		vlg_error("gethostname fail, errno[%d]", errno);
		goto err;
	}
	...
	//vlog_event_profile(a_event, vlg_DEBUG);
	return a_event;
err:
