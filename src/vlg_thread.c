...
{
	vlg_assert(a_thread,);
	vlg_profile(flag, "--thread[%p][%p][%p][%p,%p,%p,%p,%p]--",
			a_thread,
			a_thread->mdc,
			a_thread->event,
...
void vlog_thread_del(vlog_thread_t * a_thread)
{
	...
	vlg_debug("vlog_thread_del[%p]", a_thread);
    free(a_thread);
	return;
}
...
vlog_thread_t *vlog_thread_new(int init_version, size_t buf_size_min, size_t buf_size_max, int time_cache_count)
{
	...
	if (!a_thread->mdc) {
		vlg_error("vlog_mdc_new fail");
		goto err;
	}

	a_thread->event = vlog_event_new(time_cache_count);
	if (!a_thread->event) {
		vlg_error("vlog_event_new fail");
		goto err;
	}
	...
	//vlog_thread_profile(a_thread, vlg_DEBUG);
	return a_thread;
err:
