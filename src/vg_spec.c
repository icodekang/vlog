...
{
	vg_assert(a_spec,);
	vg_profile(flag, "----spec[%p][%.*s][%s|%d][%s,%ld,%ld,%s][%s]----",
		a_spec,
...
static int vlog_spec_write_mdc(vlog_spec_t * a_spec, vlog_thread_t * a_thread, vlog_buf_t * a_buf)
{
	...
	if (!a_mdc_kv) {
		vg_error("vlog_mdc_get_kv key[%s] fail", a_spec->mdc_key);
		return 0;
	}
...
static int vlog_spec_write_usrmsg(vlog_spec_t * a_spec, vlog_thread_t * a_thread, vlog_buf_t * a_buf)
{
	if (a_thread->event->generate_cmd == vlog_FMT) {
		...
		while (1) {
	      ...
	      vlog_hex_exit:
		if (rc < 0) {
			vg_error("write hex msg fail");
			return -1;
		} else if (rc > 0) {
			vg_error("write hex msg, buf is full");
			return 1;
		}
...
{
	vg_assert(a_spec,);
	vg_debug("vlog_spec_del[%p]", a_spec);
    free(a_spec);
}
...
vlog_spec_t *vlog_spec_new(char *pattern_start, char **pattern_next, int *time_cache_count)
{
	...
	vg_assert(pattern_start, NULL);
	vg_assert(pattern_next, NULL);
	...
	case '%':
		...
		if (nscan == 1) {
			...
			if (vlog_spec_parse_print_fmt(a_spec)) {
				vg_error("vlog_spec_parse_print_fmt fail");
				goto err;
			}
		} else {
		...
		if (*p == 'd') {
			if (*(p+1) != '(') {
				...
				p += nread;
				if (*(p - 1) != ')') {
					vg_error("in string[%s] can't find match \')\'", a_spec->str);
					goto err;
				}
			}
		...
		*pattern_next = p + 1;
		...
		case '%':
			...
			break;
		default:
			vg_error("str[%s] in wrong format, p[%c]", a_spec->str, *p);
			goto err;
		}
		break;
	default:
	...
	vlog_spec_profile(a_spec, vg_DEBUG);
	return a_spec;
err:
