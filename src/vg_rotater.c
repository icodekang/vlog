...
{
	vg_assert(a_rotater,);
	vg_profile(flag, "--rotater[%p][%p,%s,%d][%s,%s,%s,%ld,%ld,%d,%d,%d]--",
		a_rotater,
	...
	if (a_rotater->files) {
		int i;
		vlog_file_t *a_file;
		vg_arraylist_foreach(a_rotater->files, i, a_file) {
			vg_profile(flag, "[%s,%d]->", a_file->path, a_file->index);
		}
	}
	return;
}
...
void vlog_rotater_del(vlog_rotater_t *a_rotater)
{
	...
	if (a_rotater->lock_fd) {
		if (close(a_rotater->lock_fd)) {
			vg_error("close fail, errno[%d]", errno);
		}
	}

	if (pthread_mutex_destroy(&(a_rotater->lock_mutex))) {
		vg_error("pthread_mutex_destroy fail, errno[%d]", errno);
	}

	vg_debug("vlog_rotater_del[%p]", a_rotater);
    free(a_rotater);
	return;
}
...
vlog_rotater_t *vlog_rotater_new(char *lock_file)
{
	...
	if (pthread_mutex_init(&(a_rotater->lock_mutex), NULL)) {
		vg_error("pthread_mutex_init fail, errno[%d]", errno);
		free(a_rotater);
		return NULL;
	}
	...
	if (fd < 0) {
		vg_error("open file[%s] fail, errno[%d]", lock_file, errno);
		goto err;
	}
	...
	//vlog_rotater_profile(a_rotater, vg_DEBUG);
	return a_rotater;
err:
...
{
	vg_debug("del onefile[%p]", a_file);
	vg_debug("a_file->path[%s]", a_file->path);
	free(a_file);
}
...
static vlog_file_t *vlog_file_check_new(vlog_rotater_t * a_rotater, const char *path)
{
	...
	if (nwrite < 0 || nwrite >= sizeof(a_file->path)) {
		vg_error("snprintf fail or overflow, nwrite=[%d], errno[%d]", nwrite, errno);
		goto err;
	}
	...
	if (a_rotater->num_width != 0) {
		if (nread < a_rotater->num_width) {
			vg_warn("aa.1.log is not expect, need aa.01.log");
			goto err;
		}
	} /* else all file is ok */
...
static int vlog_rotater_add_archive_files(vlog_rotater_t * a_rotater)
{
	...
	a_rotater->files = vg_arraylist_new((vg_arraylist_del_fn)vlog_file_del);
	if (!a_rotater->files) {
	...
	if (rc == GLOB_NOMATCH) {
		...
		vg_error("glob err, rc=[%d], errno[%d]", rc, errno);
		return -1;
	}
	...
	for (; pathc-- > 0; pathv++) {
		...
		if (!a_file) {
			vg_warn("not the expect pattern file");
			continue;
		}

		/* file in list aa.00, aa.01, aa.02... */
		rc = vg_arraylist_sortadd(a_rotater->files,
					(vg_arraylist_cmp_fn)vlog_file_cmp, a_file);
		if (rc) {
			vg_error("vg_arraylist_sortadd fail");
			goto err;
		}
	}
...
static int vlog_rotater_seq_files(vlog_rotater_t * a_rotater)
{
	...
	vg_arraylist_foreach(a_rotater->files, i, a_file) {
		if (a_rotater->max_count > 0 
			&& i < vg_arraylist_len(a_rotater->files) - a_rotater->max_count) {
			/* unlink aa.0 aa.1 .. aa.(n-c) */
			rc = unlink(a_file->path);
			if (rc) {
				vg_error("unlink[%s] fail, errno[%d]",a_file->path , errno);
				return -1;
			}
			continue;
		}
	...
	if (vg_arraylist_len(a_rotater->files) > 0) { /* list is not empty */
		a_file = vg_arraylist_get(a_rotater->files, vg_arraylist_len(a_rotater->files)-1);
		if (!a_file) {
			vg_error("vg_arraylist_get fail");
			return -1;
		}

		j = vg_max(vg_arraylist_len(a_rotater->files)-1, a_file->index) + 1;
	} else {
		j = 0;
	}
	...
	if (nwrite < 0 || nwrite >= sizeof(new_path)) {
		vg_error("nwirte[%d], overflow or errno[%d]", nwrite, errno);
		return -1;
	}

	if (rename(a_rotater->base_path, new_path)) {
		vg_error("rename[%s]->[%s] fail, errno[%d]", a_rotater->base_path, new_path, errno);
		return -1;
	}
...
static int vlog_rotater_roll_files(vlog_rotater_t * a_rotater)
{
	...
	for (i = vg_arraylist_len(a_rotater->files) - 1; i > -1; i--) {
		a_file = vg_arraylist_get(a_rotater->files, i);
		if (!a_file) {
			vg_error("vg_arraylist_get fail");
			return -1;
		}
		...
		if (rename(a_file->path, new_path)) {
			vg_error("rename[%s]->[%s] fail, errno[%d]", a_file->path, new_path, errno);
			return -1;
		}
	}
...
static int vlog_rotater_parse_archive_path(vlog_rotater_t * a_rotater)
{
	...
	if (a_rotater->archive_path[0] == '\0') {
		...
		if (!p) {
			vg_error("no # in archive_path[%s]", a_rotater->archive_path);
			return -1;
		}
		...
		if (len > sizeof(a_rotater->glob_path) - 1) {
			vg_error("sizeof glob_path not enough,len[%ld]", (long) len);
			return -1;
		}
...
static void vlog_rotater_clean(vlog_rotater_t *a_rotater)
{
	...
	if (a_rotater->files) vg_arraylist_del(a_rotater->files);
		...
		char *base_path, char *archive_path, int archive_max_count)
{
	...
	if (rc) {
		vg_error("vlog_rotater_parse_archive_path fail");
		goto err;
	}

	rc = vlog_rotater_add_archive_files(a_rotater);
	if (rc) {
		vg_error("vlog_rotater_add_archive_files fail");
		goto err;
	}
	...
	if (a_rotater->mv_type == ROLLING) {
		...
		if (rc) {
			vg_error("vlog_rotater_roll_files fail");
			goto err;
		}
	} else if (a_rotater->mv_type == SEQUENCE) {
		rc = vlog_rotater_seq_files(a_rotater);
		if (rc) {
			vg_error("vlog_rotater_seq_files fail");
			goto err;
		}
	}
...
static int vlog_rotater_trylock(vlog_rotater_t *a_rotater)
{
	...
	if (rc == EBUSY) {
		vg_warn("pthread_mutex_trylock fail, as lock_mutex is locked by other threads");
		return -1;
	} else if (rc != 0) {
		vg_error("pthread_mutex_trylock fail, rc[%d]", rc);
		return -1;
	}
	...
	if (fcntl(a_rotater->lock_fd, F_SETLK, &fl)) {
		if (errno == EAGAIN || errno == EACCES) {
			...
			/* EAGAIN on linux */
			/* EACCES on AIX */
			vg_warn("fcntl lock fail, as file is lock by other process");
		} else {
			vg_error("lock fd[%d] fail, errno[%d]", a_rotater->lock_fd, errno);
		}
		if (pthread_mutex_unlock(&(a_rotater->lock_mutex))) {
			vg_error("pthread_mutex_unlock fail, errno[%d]", errno);
		}
		return -1;
	}
...
static int vlog_rotater_unlock(vlog_rotater_t *a_rotater)
{
	...
	if (fcntl(a_rotater->lock_fd, F_SETLK, &fl)) {
		rc = -1;
		vg_error("unlock fd[%s] fail, errno[%d]", a_rotater->lock_fd, errno);
	}

	if (pthread_mutex_unlock(&(a_rotater->lock_mutex))) {
		rc = -1;
		vg_error("pthread_mutext_unlock fail, errno[%d]", errno);
	}
		...
		char *archive_path, long archive_max_size, int archive_max_count)
{
	...
	vg_assert(base_path, -1);

	if (vlog_rotater_trylock(a_rotater)) {
		vg_warn("vlog_rotater_trylock fail, maybe lock by other process or threads");
		return 0;
	}

	if (stat(base_path, &info)) {
		rc = -1;
		vg_error("stat [%s] fail, errno[%d]", base_path, errno);
		goto exit;
	}
	...
	if (rc) {
		vg_error("vlog_rotater_lsmv [%s] fail, return", base_path);
		rc = -1;
	} /* else if (rc == 0) */

	//vg_debug("vlog_rotater_file_ls_mv success");

exit:
	/* unlock file */
	if (vlog_rotater_unlock(a_rotater)) {
		vg_error("vlog_rotater_unlock fail");
	}
